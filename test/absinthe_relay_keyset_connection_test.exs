defmodule AbsintheRelayKeysetConnectionTest do
  ...
  describe "encoding and decoding cursors, ensuring valid columns" do
    test "with an id column" do
      raw = %{id: 1}
      encoded = KC.key_to_cursor(raw, [:id])
      {:ok, decoded} = KC.cursor_to_key(encoded, [:id])
      assert decoded == raw
    end
    ...
    test "with multiple columns combined for uniqueness" do
      ...
      encoded = KC.key_to_cursor(raw, expected_columns)

      # If these are the exact columns we expect, it's valid
      {:ok, decoded} = KC.cursor_to_key(encoded, expected_columns)
      assert decoded == raw

      # If there are columns we don't expect, it's invalid
      expected_columns = [:id]
      {:error, :invalid_cursor} = KC.cursor_to_key(encoded, expected_columns)
